...
// Help prevent noSQL injection via validation regex
...
async function createUser(req, res) {
    ...
    try {
        ...
        if (!validateUsername(username)) {
            return res.status(400).json({ message: "Username cannot contain symbols" });
        }
...
async function updateUserProfile(req, res) {
    ...
    try {
        ...
        const { password: hashedPassword, ...otherDetails } = updatedUser._doc;
        res.status(200).json({ message: "Profile updated successfully", user: otherDetails });
    } catch (error) {
